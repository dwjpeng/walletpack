...
export default class ROXE extends Plugin {
	...
	async proxyVote(account, proxyAccount, prompt = false){
		return new Promise(async (resolve, reject) => {
			...
			const roxe = this.getSignableRoxejs(account, reject, prompt);
	...
	async changePermissions(account, keys){
		if(!keys) return;
		return new Promise(async (resolve, reject) => {
			const roxe = this.getSignableRoxejs(account, reject);
	...
	defaultToken(){ return new Token(Blockchains.ROXE, 'roxe.token', 'ROC', 'ROC', this.defaultDecimals(), MAINNET_CHAIN_ID) }
	...
	async createAccount(account, name, owner, active, roxeUsed){
		return new Promise(async (resolve, reject) => {


			const coreSymbol = account.network().systemToken().symbol;

			const net = (roxeUsed/4).toFixed(account.network().systemToken().decimals);
			const cpu = (roxeUsed-net).toFixed(account.network().systemToken().decimals);
	...
	async transfer({account, to, amount, token, memo, promptForSignature = true}){
		...
		return new Promise(async (resolve, reject) => {
			const roxe = this.getSignableRoxejs(account, reject, promptForSignature);

			const result = await roxe.transact({
				actions:[{
					account: contract,
					name:'transfer',
					authorization: [{
